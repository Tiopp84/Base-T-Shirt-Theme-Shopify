...
document.addEventListener('invalid', (function (e) {
  ...
  // Ignore forms that opt-out or are already handled by other scripts
  if (!form || form.hasAttribute('novalidate') || form.classList.contains('no-custom-validation')) return;

  e.preventDefault();

  // Find the wrapper to append the error to
  const wrapper = field.closest('.field') || field;
  
  // Check if an error message already exists for this field
  let errorMessage = wrapper.parentNode.querySelector(`.custom-client-error[data-for="${field.id || field.name}"]`);
  
  if (errorMessage) {
    // If it exists, just update the text (prevents layout thrashing)
    const textSpan = errorMessage.querySelector('.custom-client-error-text');
    if (textSpan && textSpan.textContent !== field.validationMessage) {
      textSpan.textContent = field.validationMessage;
    }
  } else {
    // Create a new error message element matching Dawn's design
    errorMessage = document.createElement('small');
    errorMessage.className = 'form__message custom-client-error';
    errorMessage.dataset.for = field.id || field.name;
    errorMessage.style.color = 'rgb(var(--color-error))';
    ...
    errorMessage.innerHTML = `
      <svg aria-hidden="true" focusable="false" class="icon icon-error" viewBox="0 0 13 13" style="width: 1.5rem; height: 1.5rem; flex-shrink: 0;">
      ...
      <span class="custom-client-error-text">${field.validationMessage}</span>
    `;

    wrapper.insertAdjacentElement('afterend', errorMessage);

    // Clear error on input (only added ONCE when element is created)
  ...
  // Always restore focus to the first invalid field
  const invalidFields = form.querySelectorAll(':invalid');
  if (invalidFields.length > 0 && invalidFields[0] === field) {
    field.focus();
  }
